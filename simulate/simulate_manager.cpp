...
int SimulateManager::get_balance(const address& addr, uint256be& val) {
    ...
    if (p == 0) {
        ...
        if (request_balance(_client, addr_str, balance_data) != 0) {
            return -1;
        }
...
int SimulateManager::get_storage(const address& addr, const bytes32& key, bytes32& value) {
    ...
    if (pp == 0) {
        ...
        if (request_storage(_client, addr_str, key_str, result_str) != 0) {
            return -1;
        }
...
int SimulateManager::get_code(const address& addr, std::shared_ptr<Code>* c) {
    std::string code_str;
    std::string addr_str = evmc_address_to_str(addr);
    if (request_code(_client, addr_str, code_str) != 0) {
        return -1;
    }
...
int SimulateManager::get_nonce(const address& addr, uint64_t& nonce) {
    ...
    if (p == 0) {
        if (request_nonce(_client, evmc_address_to_str(addr), nonce) != 0) {
            return -1;
        }
...
void SimulateManager::run_in_loop() {
    while (true) {
        ...
        while (true) {
            ...
            if (TxPool::instance()->get_tx(cur, tx) != 0) {
                // new head, txs is clear; or all pending txs simulated
                LOG(INFO) << "get tx failed:" << cur;
                break;
            }
            SimulateHost* prev = 0;
            if (cur != 0) {
            ...
            auto addr_tmp = str_to_address(tx->from);
            evmc_address_copy(&context.tx_origin, &addr_tmp);
            auto bytes32_tmp = add_bytes32(bytes32(tx->priority_fee), _base_fee);
            evmc_bytes32_copy(&context.tx_gas_price, &bytes32_tmp);
            memset(&context.block_coinbase, 0, sizeof(evmc_address));
            context.block_number = _block_number;
            context.block_timestamp = _time_stamp;
            context.block_gas_limit = _gas_limit;
            evmc_bytes32_copy(&context.block_prev_randao, &_prev_randao);
            bytes32_tmp = bytes32(_chain_id);
            LOG(INFO) << "chainid:" << bytes32_to_str(bytes32_tmp);
            evmc_bytes32_copy(&context.chain_id, & bytes32_tmp);
            evmc_bytes32_copy(&context.block_base_fee, &_base_fee);
            evmc_bytes32_copy(&context.blob_base_fee, &_base_fee);
            context.blob_hashes = 0; //fake
            context.blob_hashes_count = 0; //fake
            context.initcodes = 0;
            context.initcodes_count = 0; // ?
            if (_hosts.size() <= cur) {
            ...
            if (tx->gas <= static_cast<uint64_t>(gas1) || balance < uint256_to_bytes32(tx->gas * tx->priority_fee)) {
                // failed
                continue;
            }
            ...
            LOG(INFO) << "end to simulte:" << cur << "[status_code=" << res.status_code << "][gas left=" << res.gas_left << "][gas refund=" << res.gas_refund;
            if (_hosts[cur]->error()) {
                _change_idx.store(0);
                continue;
            }
